...
fn test_uppercase_nguoi_produces_uppercase_nguoi() {
    ...
    assert!(
        output
            .chars()
            .all(|c| !c.is_alphabetic() || c.is_uppercase() || !c.is_ascii()),
        "Expected all uppercase, got: {}",
        output
    );
}
...
fn test_mixed_case_nguoi_produces_mixed_case() {
    ...
    assert!(
        output.chars().next().unwrap().is_uppercase(),
        "Expected first char uppercase, got: {}",
        output
    );
}
