...
fn test_executor_basic() {
    ...
    println!(
        "Config has {} transform rules",
        config.transform_rules.len()
    );
    ...
    println!(
        "  transform_history: {} records",
        ctx2.transform_history.len()
    );
    ...
    if ctx2.syllable_buffer == "â" {
        ...
        println!(
            "✗ Transform FAILED! Expected 'â', got '{}'",
            ctx2.syllable_buffer
        );
    }
}
