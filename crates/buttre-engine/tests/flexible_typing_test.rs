...
fn test_permutation_tuongwf() {
    ...
    assert_eq!(
        result, "tường",
        "tuongwf should produce tường with flexible typing"
    );
}
...
fn test_permutation_truwowngf() {
    ...
    assert_eq!(
        result, "trường",
        "truwowngf should produce trường with flexible typing"
    );
}
...
fn test_permutation_truongwf() {
    ...
    assert_eq!(
        result, "trường",
        "truongwf should produce trường with flexible typing"
    );
}
...
fn test_free_tone_nearest_vowel() {
    ...
    assert_eq!(
        result, "òa",
        "oasf: last tone (grave) on oa vowel cluster → òa"
    );
}
...
fn test_combined_flexible_typing() {
    ...
    assert!(
        result == "thường" || result == "thươngf",
        "thuwowfngf should produce thường or thươngf (got {})",
        result
    );
}
...
fn test_backward_compatibility_strict_mode() {
    ...
    assert_eq!(
        result, "tường",
        "tuongwf → tường via compose (no permutation stage needed)"
    );
}
...
fn test_permutation_disabled() {
    ...
    assert_eq!(
        result, "tường",
        "tuongwf → tường via compose (allow_permutation flag is now a no-op)"
    );
}
...
fn test_multiple_marks_permutation() {
    ...
    assert_eq!(
        result, "uờ",
        "uowf should produce uờ (w after o transforms o only)"
    );
}
...
fn test_free_tone_single_vowel() {
    ...
    assert_eq!(
        result2, "sa",
        "sa should remain sa (s is initial consonant, not tone)"
    );
...
fn create_config_with_tone_style(
    style: buttre_engine::pipeline::config::ToneStyle,
) -> PipelineConfig {
    let mut config = buttre_engine::pipeline::telex_config();
...
fn test_tone_style_old_hoa() {
    ...
    assert_eq!(
        result, "hòa",
        "Old style: hoaf should produce hòa (tone on first vowel)"
    );
}
...
fn test_tone_style_new_hoa() {
    ...
    assert_eq!(
        result, "hoà",
        "New style: hoaf should produce hoà (tone on second vowel)"
    );
}
...
fn test_tone_style_old_thuy() {
    ...
    assert_eq!(
        result, "thủy",
        "Old style: thuyr should produce thủy (tone on first vowel)"
    );
}
...
fn test_tone_style_new_thuy() {
    ...
    assert_eq!(
        result, "thuỷ",
        "New style: thuyr should produce thuỷ (tone on second vowel)"
    );
}
...
fn test_tone_style_old_hoe() {
    ...
    assert_eq!(
        result, "hóe",
        "Old style: hoes should produce hóe (tone on first vowel)"
    );
}
...
fn test_tone_style_new_hoe() {
    ...
    assert_eq!(
        result, "hoé",
        "New style: hoes should produce hoé (tone on second vowel)"
    );
}
...
fn test_tone_style_default_is_old() {
    ...
    assert_eq!(
        config.tone_style,
        buttre_engine::pipeline::config::ToneStyle::Old,
