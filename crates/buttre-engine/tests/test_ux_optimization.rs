...
fn test_ux_auto_correct_nuoc() {
    ...
    assert_eq!(result, "nuòc", "nuocf → nuòc under compose (no auto_correct_uo)");
}
...
fn test_ux_auto_correct_cuoc() {
    ...
    assert_eq!(result, "cuóc", "cuocs → cuóc under compose (no auto_correct_uo)");
}
...
fn test_ux_auto_correct_skips_existing_horn() {
    ...
    assert!(result.contains('ư') || result.contains('ơ'),
        "nguowif should contain horn marks (explicit w), got: {}", result);
}
...
fn test_ux_auto_correct_uppercase() {
    ...
    config.tone.auto_correct_uo = true; // no-op
    let result = type_word("NGUOIf", &config);
    assert!(!result.is_empty(), "NGUOIf should produce output, got: {}", result);
    assert!(result.chars().next().map_or(false, |c| c.is_uppercase()),
        "NGUOIf should produce uppercase output, got: {}", result);
}
...
fn test_ux_combined_all_features() {
    ...
    assert!(!result.is_empty(), "nguois should produce output, got: {}", result);
}
...
fn test_ux_reposition_closed_syllable() {
    ...
    assert_eq!(result.len(), "oán".len(), "Result should have same length as expected");
}
...
fn test_ux_free_marking_long_word() {
    ...
    assert!(!result.is_empty(), "thuongs should produce output, got: {}", result);
}
...
fn test_ux_auto_correct_only_with_tone() {
    ...
    assert_eq!(result, "nguoi",
        "nguoi without tone should remain nguoi");
}
...
fn test_ux_vni_auto_correct() {
    ...
    assert!(!result.is_empty(), "VNI nguoi2 should produce output, got: {}", result);
    assert!(!result.contains('ư') && !result.contains('ơ'),
        "VNI nguoi2 without explicit 7 should not have horn marks, got: {}", result);
}
