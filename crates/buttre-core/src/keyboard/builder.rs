...
use buttre_engine::pipeline::config::{LookupSettings, PipelineConfig};
    ...
    pub fn build(self) -> anyhow::Result<Keyboard> {
        ...
        let mut pipeline_config = if let Some(pc) = self.pipeline_config {
            ...
            return Err(anyhow::anyhow!(
                "Config not set (neither TOML Config nor PipelineConfig provided)"
            ));
        };
