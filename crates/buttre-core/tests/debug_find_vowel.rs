...
fn debug_huyeenf_with_presets_telex() {
    ...
    let input = "huyeenf";
    println!("\n=== Processing '{}' with presets::telex_config() ===", input);
    
    for ch in input.chars() {
        let actions = executor.process(ch);
        println!("Input '{}': syllable='{}', actions={:?}", ch, executor.syllable(), actions);
    }
    
    println!("Final syllable: '{}'", executor.syllable());
    assert_eq!(executor.syllable(), "huyền", "Should be 'huyền' not 'huỳên'");
}
...
fn debug_huyeenf_with_core_telex() {
    ...
    let input = "huyeenf";
    println!("\n=== Processing '{}' with telex::build_config() ===", input);
...
fn debug_huyeenf_with_composition() {
    ...
    let input = "huyeenf";
    println!("\n=== Processing '{}' with telex::build_config() + composition ===", input);
