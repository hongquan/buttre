...
use crate::platforms::windows::common::{send_backspaces, send_string, send_replacement};
...
impl QueueProcessor {
    ...
    pub fn enqueue(&self, event: KeyEvent) -> bool {
        ...
        if self.queue.len() >= self.max_queue_size {
            warn!("Queue full ({} events), dropping keystroke", self.max_queue_size);
            return false;
        }
                        ...
                        for action in actions {
                            match action {
                                Action::ShowCandidates { candidates, input } => {
                                    ...
                                    use crate::platforms::windows::common::{show_candidates, get_candidates_text_len};
                                ...
                                Action::HideCandidates => {
                                    use crate::platforms::windows::common::{hide_candidates, get_candidates_text_len};
